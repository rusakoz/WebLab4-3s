      ...
      <div class="auth-wrapper-container">
        <div class="login-container">
          ...
          <div class="form-control" :class="{ invalid: !form.password.valid && form.password.touched }">
            <label>Пароль</label>
            <input class="password" type="password" id="firstPassword" placeholder="Введите пароль"
              v-model="form.password.value" @blur="form.password.blur">
          ...
          <div class="form-control" :class="{ invalid: !form.equalPass && form.passwordTwo.touched }">
            ...
            <input class="password" type="password" placeholder="Повторите пароль" v-model="form.passwordTwo.value"
              @blur="form.passwordTwo.blur">
            <small v-if="form.passwordTwo.errors.required && form.passwordTwo.touched" class="errors">Введите
              пароль</small>
            ...
            <small v-else-if="!form.passwordTwo.errors.minLength && !form.equalPass.valid && form.passwordTwo.touched"
              class="errors">
              Пароли не совпадают.
            </small>
          </div>
...
async function submit() {
  ...
  const res = await useFetchPost("/user/reg",
                                            {
                                              name: form.name.value,
                                              password: form.password.value
                                            }
  ).catch((err) => {
    regErrText.value = "Ошибка регистрации(" + err + ")"
    regErr.value = true
  })
  ...
  if (res.status === 200 || res.status === 201) {
    ...
    if (hasError) {
      regErrText.value = resJson.error
      regErr.value = true
    } else {
      router.push('/login')
    }
  } else {
    regErrText.value = "Ошибка регистрации(Статус: " + res.status + ")"
    regErr.value = true
  }
}
